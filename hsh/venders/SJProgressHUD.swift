...
public extension SJProgressHUD {
     ...
     */
    public static func showWaitingWithImages(_ images : Array<UIImage>, timeInterval : TimeInterval = 2) {
...
private let windowBgColor = UIColor(red: 0, green: 0, blue: 0, alpha: 0.80)
...
open class SJProgressHUD : NSObject {
    ...
    static open func showWaitWithImages(_ images : Array<UIImage>, timeInterval : TimeInterval) {
        ...
        let frame = CGRect(x: (hCREEN_WIDTH - 130) / 2, y: (hCREEN_HEIGHT - 75) / 2, width: 130, height: 75)
        ...
        let frame1 = CGRect.init(x: 0, y: 0, width: hCREEN_WIDTH, height: hCREEN_HEIGHT)
        _ = SJProgressHUD.createWindow(frame1, view: imageView)
        
    }
    static open func showWaitingWithText(_ text: String, autoRemove: Bool) {
        
        let view = UIView(frame: CGRect(x: 0, y: 0, width: 125, height: 92))
        view.backgroundColor = bgColor(0.50)
        view.layer.cornerRadius = 10
        
//        let activity = UIActivityIndicatorView(activityIndicatorStyle: .white)
        let activity = UIActivityIndicatorView(activityIndicatorStyle: UIActivityIndicatorViewStyle.white)
        if !text.isEmpty {
            activity.frame = CGRect(x: 45.5, y: 16, width: 40, height: 40)
            
            let lable = SJProgressHUD.createLable()
            lable.frame = CGRect(x: 0, y: 40, width: 125, height: 45)
            lable.text = text
            view.addSubview(lable)
        }else{
            activity.frame = CGRect(x: 37.5, y: 26, width: 40, height: 40)
        }
    ...
    static open func onlyText(_ text: String, autoRemove: Bool) {
        ...
        if autoRemove {
            let sel = #selector(removeHUD(_:))
            perform(sel, with: window, afterDelay: 1)
        }
    }
